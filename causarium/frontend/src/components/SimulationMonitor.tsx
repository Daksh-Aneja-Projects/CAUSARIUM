    ...
    <div className="flex flex-col h-full text-gray-200 p-8 max-w-5xl mx-auto animate-fade-in">
      <div className="mb-8">
        <h1 className="text-3xl font-light text-white mb-2 flex items-center">
          {status === 'DISCOVERY' ? 'Running Discovery' : 'Simulation Running'}
          <span className="ml-4 flex space-x-1">
            <span className="w-2 h-2 bg-[#00D9FF] rounded-full animate-ping"></span>
          </span>
        </h1>
        <p className="text-gray-400 font-mono text-sm">
          ID: {simulationId} | {isConnected ? 'CONNECTED' : 'CONNECTING...'} | Status: {status}
        </p>
      </div>
      ...
      <div className="glass-panel p-8 rounded-2xl shadow-2xl mb-8 relative overflow-hidden">
        <div className="absolute top-0 left-0 w-full h-1 bg-[#222]">
          <div className="h-full bg-gradient-to-r from-[#6C63FF] to-[#00D9FF] transition-all duration-200 ease-out" style={{ width: `${progress}%` }}></div>
        </div>

        <div className="flex justify-between items-end mb-6 mt-2">
          <div className="text-6xl font-light text-white">{progress}%</div>
          <div className="text-right">
            <div className="text-[#00D9FF] font-mono uppercase tracking-widest text-sm animate-pulse">
              {status === 'DISCOVERY' ? 'Extracting Causality' : `Timeline ${currentRun}`}
            </div>
          </div>
        </div>
        ...
        {Object.keys(outcomes).length > 0 && (
          <div className="flex flex-wrap gap-2 mb-4">
            {Object.entries(outcomes).map(([outcome, n]) => (
              <span key={outcome} className="text-xs font-mono px-2 py-1 rounded bg-[#0A0A0F] border border-[#222] text-gray-400">
                {outcome} <span className="text-[#6C63FF]">×{n}</span>
              </span>
            ))}
          </div>
        )}
        ...
        <div className="h-64 bg-[#0A0A0F] border border-[#333] rounded-lg p-4 font-mono text-xs overflow-y-auto flex flex-col-reverse">
          <div>
            {events.map((ev, i) => (
              <div key={i} className="mb-1.5 pb-1.5 border-b border-[#1a1a24] last:border-0">
                <span className="text-[#6C63FF] mr-3">[{new Date(ev.timestamp).toLocaleTimeString()}]</span>
                <span className={ev.raw?.black_swan ? 'text-[#FF3366]' : ev.type.includes('complete') ? 'text-[#00D9FF]' : 'text-gray-400'}>
                  {ev.message}
                </span>
              </div>
            ))}
      ...
      <div className="text-center text-gray-500 text-sm">
        Simulating interactions across parallel timelines through real causal physics.
      </div>
    </div>
  );
};
